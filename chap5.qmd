...
Ein winziger p Wert bedeutet lediglich, dass ein Effekt mit hoher Sicherheit nicht exakt null ist. Über den echten Nutzen in der realen Welt sagt diese Metrik wenig aus. Cohen kritisierte diese Verwechslung scharf und betonte, dass eine quantitative Wissenschaft ihr primäres Ziel verfehlt, wenn sie Entscheidungen auf p Werte stützt anstatt auf die messbare Größe von Effekten (Cohen, 1990).

### Beispiel SmartRail: Das hochsignifikante Nichts
Die Bahn investiert 180 Millionen Euro in den bundesweiten SmartRail Rollout. Eine gigantische Studie mit 200.000 ICE Fahrten beweist: SmartRail verbessert die Pünktlichkeit statistisch hochsignifikant, p < 0.001. Das Projektteam präsentiert den winzigen p Wert als Beleg für einen enormen Erfolg.
...
def server(input, output, session):
    ...
    def relevanz_plot():
        ...
        ax.set_xlim(0, max(schwelle * 1.6, eff * 1.4, 50))
        ...
        ax.set_title(f"Hochsignifikant, aber: {status}",
