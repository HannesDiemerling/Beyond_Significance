...
def server(input, output, session):
    ...
    def dual_plot():
        ...
        ymin = min(means) - 4 * ci - 0.3
        ymax = max(means) + 4 * ci + 0.3
        ax2.set_ylim(max(0, ymin), ymax)
        ax2.set_xticks(x_pos)
        ax2.set_xticklabels(labels, fontsize=11)
        ax2.set_ylabel("Durchschnittliche Verspätung (Minuten)")
        ax2.set_title(
            f"Effekt: {DIFF*60:.0f} Sek | 95% KI: ±{ci:.2f} Min",
            fontsize=10
        )
        ax2.spines['top'].set_visible(False)
        ax2.spines['right'].set_visible(False)

        # Manuelle Abstände statt tight_layout für mehr Stabilität
    ...
    def stats_panel():
        ...
        return ui.div(
            ...
            ui.tags.p(f"95% KI: ±{ci:.2f} Minuten", style="margin:2px 0;"),
