---
title: "Fehlinterpretationen und binäres Denken"
format: html
---
...
# Größere Grundschrift, damit der Text auch dann lesbar bleibt, wenn das
...
z_old = np.random.standard_normal(N)
z_new = np.random.standard_normal(N)
z_old -= z_old.mean()
z_new -= z_new.mean()
old_muc = MEAN_OLD + 3.0 * z_old
new_muc = MEAN_NEW + 3.0 * z_new
...
def server(input, output, session):
    ...
    def get_hamburg_data(sd):
        ...
        z_old -= z_old.mean()
        z_new -= z_new.mean()
        old_hb = MEAN_OLD + sd * z_old
        new_hb = MEAN_NEW + sd * z_new
...
<nav class="workshop-nav">
  ...
  <a class="wnav-next" href="chap2.html"><span class="wnav-dir">Weiter →</span><span class="wnav-sub">2.2 p Wert und Stichprobengröße</span></a>
</nav>
